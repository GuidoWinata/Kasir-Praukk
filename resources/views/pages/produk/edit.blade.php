...
@section('content')
    @include('layouts.navbars.auth.topnav', ['title' => 'Edit Produk'])
    <div class="row mt-4 mx-4">
        <div class="col-md-7">
            <div class="card pb-1">
                ...
                <div class="card-body">
                    <form action="{{ route('produk.doUpdate', ["id" => $produk->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <label for="nama"><p class="fw-bold m-0">Nama Produk</p></label>
                        <input type="text" name="nama" class="form-control form-control-lg mb-3 @error('nama') is-invalid @enderror" id="nama" placeholder="Nama Produk" value="{{ old('nama', $produk->NamaProduk) }}" required>
                        ...
                        <div class="input-group mb-3">
                            ...
                            <input type="hidden" name="harga" id="harga_raw" value="{{ old('harga', $produk->Price) }}">
                        </div>
                        <label for="stok"><p class="fw-bold m-0">Stok</p></label>
                        <input type="number" name="stok" class="form-control w-25 form-control-lg mb-3 @error('stok') is-invalid @enderror" id="stok" placeholder="Stok" value="{{ old('stok', $produk->Stok) }}" required>
